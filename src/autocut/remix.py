    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "4. 后续顺序要符合短视频成片逻辑：开头钩子/产品身份 -> 核心卖点 -> 使用场景/材质证明 -> 价格或下单理由；不要把无上下文的碎句、重复话术、语义跳跃很大的句子硬拼在一起。",
            "5. 如果某句虽然可用但像直播中途的承接话，只能放在前后语义能接上的位置，不能作为开头。",
            "6. reason 字段需要说明：为什么第一句不突兀，以及整体排序如何保证成片合理。",
            "",
            "业务背景：",
            ...
            "",
            "请只输出 JSON，格式如下：",
            '{"ordered_ids":["s001","s008","s003"],"reason":"一句话说明排序逻辑"}',
            "",
            "可用字幕句子：",
            "\n".join(usable_lines),
            "",
