...
class TestBusinessPage(unittest.TestCase):
    ...
    def test_business_html_route(self):
        ...
        self.assertIn('id="remix-duration" value="50"', response.text)
    ...
    def test_business_js_route(self):
        ...
        self.assertIn("btn-delete-selected-jobs", response.text)
        self.assertIn("response.status === 204", response.text)
